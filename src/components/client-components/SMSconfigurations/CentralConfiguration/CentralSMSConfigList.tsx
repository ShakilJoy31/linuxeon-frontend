...
import { useDeleteCentralSMSMutation, useGetAllCentralSMSQuery, useGetCentralSMSStatsQuery, useTestCentralSMSMutation, useToggleCentralSMSStatusMutation } from '@/redux/api/sms-configurations/centralSmsApi';
import AddEditCentralSMSModal from './AddEditCentralSMSModal';
import CentralSMSDetailsModal from './CentralSMSDetailsModal';

const CentralSMSConfigurationPage: React.FC = () => {
    ...
    const [viewModal, setViewModal] = useState<{ isOpen: boolean; sms: CentralSMS | null }>({
        isOpen: false,
        sms: null,
    });

    const [deleteModal, setDeleteModal] = useState<{ isOpen: boolean; sms: CentralSMS | null }>({
        isOpen: false,
        sms: null,
    });

    const [isAddSMSModalOpen, setIsAddSMSModalOpen] = useState(false);
    const [smsToEdit, setSmsToEdit] = useState<CentralSMS | null>(null);
    ...
    const { data: statsData } = useGetCentralSMSStatsQuery({});
    const [deleteSMS] = useDeleteCentralSMSMutation();
    const [testSMS] = useTestCentralSMSMutation();
    const [toggleSMSStatus] = useToggleCentralSMSStatusMutation();
    ...
    // Handle view action
    const handleView = (sms: CentralSMS) => {
        setViewModal({ isOpen: true, sms });
    };

    // Handle edit action
    const handleEdit = (sms: CentralSMS) => {
        setSmsToEdit(sms);
        setIsAddSMSModalOpen(true);
    };

    // Handle delete confirmation
    const handleDeleteClick = (sms: CentralSMS) => {
    ...
    const handleDelete = async () => {
        ...
        try {
            await deleteSMS(deleteModal.sms.id).unwrap();
    ...
    const handleTestSMS = async (sms: CentralSMS) => {
        try {
            const testData = {
                phoneNumber: "+8801234567890",
                customMessage: sms.message || "This is a test message from central SMS configuration system."
            };
    ...
    // Toggle SMS status
    const handleToggleStatus = async (sms: CentralSMS) => {
        try {
            await toggleSMSStatus(sms.id).unwrap();
            toast.success(`SMS configuration ${sms.status === 'active' ? 'deactivated' : 'activated'} successfully`);
            refetch();
        } catch (error) {
            console.log(error);
    ...
    const addSerialNumbers = (data: CentralSMS[]) => {
            ...
            >
                <div className="mb-6">
                    <h1 className={`text-2xl md:text-3xl font-bold ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
                        Central SMS Configuration
                    </h1>
                    <p className={`mt-1 ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>
                        Manage global SMS configurations for all services
                    </p>
                </div>
            </motion.div>
            ...
            {stats && (
                ...
                >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        {[
                            { icon: Globe, color: "blue", label: "Total Configurations", value: stats.totalSMS },
                            ...
                            { icon: FileText, color: "purple", label: "Unique Services", value: stats.uniqueServices },
                        ].map((stat, index) => (
                            <motion.div
                                key={index}
                                initial={{ opacity: 0, y: 20 }}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div>
                        ...
                        <div className="relative">
                            ...
                            <input
                                ...
                                placeholder="Search by app name, sender ID..."
                    ...
                    <div className="flex items-end">
                        <motion.button
                            ...
                            className={`w-full cursor-pointer px-4 py-2 rounded-lg transition-colors duration-300 flex items-center justify-center gap-2 ${
                                theme === 'dark'
                        ...
                        <div className="overflow-x-auto">
                            <table className="min-w-full divide-y transition-colors duration-300">
                                <thead className={theme === 'dark' ? 'bg-gray-900/50' : 'bg-gray-50'}>
                                    <tr>
                                        {['#', 'App Name / Message', 'Sender ID', 'Type', 'Created', 'Actions'].map((header, index) => (
                                            <th
                                                key={index}
                                    ...
                                    {addSerialNumbers(smsConfigs).map((sms, index) => (
                                        ...
                                        >
                                            ...
                                            <td className="px-6 py-4">
                                                <div>
                                                    <div className={`text-sm font-medium transition-colors duration-300 flex items-center gap-2 ${
                                                        theme === 'dark' ? 'text-white' : 'text-gray-900'
                                                    }`}>
                                                ...
                                                <div className="flex items-center gap-2">
                                                    {[
                                                        ...
                                                        <motion.button
                                                            ...
                                                            whileTap={{ scale: 0.9 }}
                                                            onClick={btn.action}
                                                            className={`p-1.5 cursor-pointer rounded transition-colors duration-300 ${
                                                                theme === 'dark'
...
export default CentralSMSConfigurationPage;